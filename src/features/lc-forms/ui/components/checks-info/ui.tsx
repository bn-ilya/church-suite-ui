...
export const ChecksInfo: FC<IChecksInfoProps> = ({
  refCallback,
  registerSenderName,
  defaultSenderName,
  ...props
  ...
  return (
    <div className="col-span-2">
      ...
      <Accordion isCompact variant="splitted" className="mt-2 col-span-2 px-0">
        <AccordionItem
          aria-label=""
          title={<div className="text-sm">Не получается пркирепить чек?</div>}
        >
          <Divider />
          <div className="flex flex-col py-6 gap-2">
            <p className="text-sm">
              Введите точное имя отправителя (из чека), мы свяжимся с вами
              позже, чтобы получить чек другим способом
            </p>
            <Input
              {...registerSenderName}
              label=""
