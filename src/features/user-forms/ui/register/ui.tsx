...
export const UserRegisterForm = () => {
  const { onSubmit, isLoading, errorMsg, errorCode, data } =
    useCreateOnSubmit();
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<IRegDataUser>({ mode: "onChange" });
  ...
  return (
    <div className="max-w-xl w-full mx-auto px-6">
      ...
      >
        <Input
          startContent={<span className="text-[12px] text-zinc-400">+7</span>}
          isRequired
          {...register("phone", {
            required: "Заполниие телефон",
            onChange: handleChangePhone,
            pattern: {
              value: /^[0-9+-]+$/,
              message: "Используются недопустимые символы",
            },
            minLength: {
              value: 10,
              message: "Недостаточно символов. Требуется 10",
            },
            maxLength: { value: 10, message: "Лишние символы. Требуется 10" },
          })}
